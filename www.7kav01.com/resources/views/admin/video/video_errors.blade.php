...
layui.use(['table','layer','form','jquery'], function(){
	...
	var tableIns = table.render({
		  ...
		  ,url: "{{url('video/get_errors_list')}}" //设置异步接口
		  ,page: true
		  ,limit: 30
		  ,id:'res_list'
		});
	...
	table.on('tool(res_list)', function(obj){
		...
		if(layEvent === 'sure'){
			...
			layer.confirm('确认执行该操作么？',{icon: 3,title:'提示'}, function(index){
				layer.close(index);
				$.post("{{url('video_errors_del')}}",{_token:_token,id:data.id,state:1},function(res){
					if(res.status==2){
		...
		if(layEvent === 'del'){
			layer.confirm('确认执行该操作么？',{icon: 3,title:'提示'}, function(index){
				layer.close(index);
				$.post("{{url('video_errors_del')}}",{_token:_token,id:data.id,state:2},function(res){
					if(res.status==2){
						layer.msg(res.msg,{icon: 2,time: 1500,anim: 6,shade:0.3});
					}						
					if(res.status==1){
