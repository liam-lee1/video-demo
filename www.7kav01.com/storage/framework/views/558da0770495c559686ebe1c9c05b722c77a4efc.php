...
<div class="bg-g">
	<?php if(!empty($meb)): ?>
		<div class="show-box mb-1">
			...
			<div class="msg-item">
				<span>会员账号：<?php echo e($meb['user']); ?></span>
				...
				<span>当前经验值：<?php echo e($meb['exp']); ?></span>
			</div>
			<div class="msg-item">
				<span>付费会员：<?php if($meb['ispaid']==1): ?> 是 <?php else: ?> 否 <?php endif; ?>
				<a href="../category" class="layui-btn layui-btn-danger layui-btn-sm"><?php if($meb['ispaid']==1): ?> 续费 <?php else: ?> 点击开通 <?php endif; ?></a>
				</span>
				<?php if($meb['active_time']>0): ?>
					<span>会员有效期：<?php echo e(date('Y-m-d H:i:s',$meb['active_time'])); ?></span>
				<?php endif; ?>
				<span>账户余额：<?php echo e($meb['balance']); ?> <button class="layui-btn layui-btn-sm layui-btn-warm withdraw-btn">提现</button><button class="layui-btn layui-btn-sm layui-btn-normal zfb">支付宝绑定</button></span>
			</div>
		</div>
	<?php endif; ?>
	<!--
	...
	<div class="show-box mb-1">
		...
		<div class="msg-item">
			<p>推广赚经验积分</p>
			<p><span>把本站分享给您的好友，好友访问增加<?php echo e(tpCache('para.recom_click_credit')); ?>积分<?php echo e(tpCache('para.recom_click_exp')); ?>经验，好友注册登录您将会获得<font color="#3f51b5"><?php echo e(tpCache('para.recom_enroll_credit')); ?>积分</font>和<font color="#3f51b5"><?php echo e(tpCache('para.recom_enroll_exp')); ?>经验值</font></span>，用户充值即可获取30%现金返利哦！</p>
			<p><font color="#f9010a">注意：您的好友每推荐一个他的好友加入，您还将获得<?php echo e(tpCache('para.recom_sec_enroll_credit')); ?>积分+<?php echo e(tpCache('para.recom_sec_enroll_exp')); ?>经验值。</font></p>
			<?php if(!empty($meb)): ?>
			<p><font color="#3f51b5">你的推广链接：</font>
				<input type="text" class="layui-input" id="copyVal" value="<?php echo e(url('/s')); ?>/<?php echo e($key); ?>" />
