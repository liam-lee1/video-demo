...
layui.use(['table','layer','form','jquery','rate'], function(){
	...
	var tableIns = table.render({
		  ...
		  ,url: "<?php echo e(url('account/get_mebList')); ?>" //设置异步接口
		  ,page: true
		  ,limit: 30
		  ,id:'meb_list',
	...
	table.on('tool(meb_list)', function(obj){
		...
		if(layEvent === 'edit'){
			layer.open({
				  ...
				  shadeClose: true,
				  area:['480px','450px'],
				  content: "<?php echo e(url('manage/meb_mation')); ?>?uid="+data.id
			  });
		}
		...
		if(layEvent === 'account'){
			layer.open({
				  ...
				  content: "<?php echo e(url('manage/account_detail')); ?>?openid="+data.openid
			  });
		}
		...
		if(layEvent === 'balance'){
			layer.open({
				  ...
				  shadeClose: true,
				  skin: 'layui-layer-rim', //加上边框
				  content: "<?php echo e(url('manage/balance_detail')); ?>?user="+data.user
			  });
		}
		...
		if(layEvent === 'del'){
			layer.confirm('确认删除该会员账户么？',{icon: 3,title:'提示'}, function(index){
				$.post("<?php echo e(url('account/del_account')); ?>/"+data.id,{_token:_token},function(res){
					layer.close(index);
					if(res.status==2){
		...
		if(layEvent === 'freeze'){
			layer.confirm('确认冻结该账户么？',{icon: 3,title:'提示'}, function(index){
				$.post("<?php echo e(url('account/meb_freeze')); ?>/"+data.id,{_token:_token},function(res){
					layer.close(index);
					if(res.status==2){
						layer.msg(res.msg,{icon: 2,time: 1500,anim: 6,shade:0.3});
					}						
		...
		if(layEvent === 'thaw'){
			layer.confirm('确认解冻该账户么？',{icon: 3,title:'提示'}, function(index){
				$.post("<?php echo e(url('account/meb_thaw')); ?>/"+data.id,{_token:_token},function(res){
					layer.close(index);
					if(res.status==2){
						layer.msg(res.msg,{icon: 2,time: 1500,anim: 6,shade:0.3});
					}						
