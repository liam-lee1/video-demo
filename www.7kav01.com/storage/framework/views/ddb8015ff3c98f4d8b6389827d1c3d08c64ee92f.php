...
layui.use(['table','layer','form','jquery'], function(){
	...
	var tableIns = table.render({
		  ...
		  ,url: "<?php echo e(url('info/get_notice_list')); ?>" //设置异步接口
		  ,page: true
		  ,limit: 30
		  ,id:'res_list'
		});
	...
	table.on('tool(res_list)', function(obj){
		...
		if(layEvent === 'edit'){
			layer.open({
			  ...
			  content: "<?php echo e(url('info/notice_xg')); ?>?id="+data.id,
			  cancel: function(){ 
		...
		if(layEvent === 'del'){
			layer.confirm('确认删除该公告么？',{icon: 3,title:'提示'}, function(index){
				$.post("<?php echo e(url('info/del_notice')); ?>",{_token:_token,id:data.id},function(res){
					layer.close(index);
					if(res.status==2){
		...
		if(layEvent === 'hide'){
			layer.confirm('确认隐藏该视频么？',{icon: 3,title:'提示'}, function(index){
				$.post("<?php echo e(url('info/notice_hide')); ?>",{_token:_token,id:data.id,state:1},function(res){
					layer.close(index);
					if(res.status==2){
						layer.msg(res.msg,{icon: 2,time: 1500,anim: 6,shade:0.3});
					}						
		...
		if(layEvent === 'show'){
			layer.confirm('确认显示该公告么？',{icon: 3,title:'提示'}, function(index){
				$.post("<?php echo e(url('info/notice_hide')); ?>",{_token:_token,id:data.id,state:0},function(res){
					layer.close(index);
					if(res.status==2){
						layer.msg(res.msg,{icon: 2,time: 1500,anim: 6,shade:0.3});
					}						
	...
	$('.add_new').click(function(){
		layer.open({
			  ...
			  shadeClose: true,
			  skin: 'layui-layer-rim',
			  content: "<?php echo e(url('info/notice')); ?>",
			  cancel: function(){ 
					window.location.reload();
				}
		  });
