...
<head>
  ...
  <link href="<?php echo e(asset('Fliter/Fliter.css')); ?>" rel="stylesheet" />
	<script src="<?php echo e(asset('Fliter/Filter.js')); ?>"></script>
</head>
<style>
...
layui.use(['table','layer','form','rate'], function(){
	...
	var tag = <?php if(isset($_GET['tag'])): ?><?php echo e($_GET['tag']); ?><?php else: ?>''<?php endif; ?>,
		type = <?php if(isset($_GET['type'])): ?><?php echo e($_GET['type']); ?><?php else: ?>''<?php endif; ?>;
	...
	var tableIns = table.render({
		  ...
		  ,url: "<?php echo e(url('video/get_video_list')); ?>" //设置异步接口
		  ,page: true
		  ,limit: 30
		  ,id:'video_list',
	...
	var data = <?php if(!empty($tag)): ?><?php echo $tag; ?><?php else: ?>[]<?php endif; ?>;
	...
	table.on('tool(video_list)', function(obj){
		...
		if(layEvent === 'edit'){
			layer.open({
				  ...
				  content: "<?php echo e(url('video/edit_video')); ?>?vid="+data.id,
				  cancel: function(){ 
		...
		if(layEvent === 'del'){
			layer.confirm('确认删除该视频么？',{icon: 3,title:'提示'}, function(index){
				$.post("<?php echo e(url('video/del_video')); ?>/"+data.id,{_token:_token},function(res){
					layer.close(index);
					if(res.status==2){
		...
		if(layEvent === 'hide'){
			layer.confirm('确认隐藏该视频么？',{icon: 3,title:'提示'}, function(index){
				$.post("<?php echo e(url('video/change_video_state')); ?>/"+data.id,{_token:_token,state:1},function(res){
					layer.close(index);
					if(res.status==2){
						layer.msg(res.msg,{icon: 2,time: 1500,anim: 6,shade:0.3});
					}						
		...
		if(layEvent === 'show'){
			layer.confirm('确认显示该视频么？',{icon: 3,title:'提示'}, function(index){
				$.post("<?php echo e(url('video/change_video_state')); ?>/"+data.id,{_token:_token,state:0},function(res){
					layer.close(index);
					if(res.status==2){
						layer.msg(res.msg,{icon: 2,time: 1500,anim: 6,shade:0.3});
					}						
	...
	$('.add_new').click(function(){
		layer.open({
			  ...
			  shadeClose: true,
			  skin: 'layui-layer-rim',
			  content: "<?php echo e(url('video/add_video')); ?>",
			  cancel: function(){ 
					window.location.reload();
				}
		  });
